...
## 前言
---
本项目为一个轻量级点对点可靠串行通信协议，依赖于list库与fifo库，可在本人github中找到源码，本协议属于运输层协议采取类似TCP协议，协议所占内存小，执行效率高可用在嵌入式设备当中，移植方便无需依赖外部库。具有超时重传机制，校验失败重传机制，重复检测机制，在确定连接状态情况下可确保达到仅送达一次效果。协议使用内存可自由配置。
